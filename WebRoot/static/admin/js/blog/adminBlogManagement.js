...
layui.define(['element', 'laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function doSearch(layer, index, pageNow) {
        ...
        ajax({
            url: reqMap.blog.adminAdminList,
            type: "GET",
            data: params,
            success: function (resp) {
                if (resp.success) {
                    ...
                    for (var i in resp.data.list) {
                        ...
                        if ("20" === item.state) {
                            html.append('<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.funcs.transferState(' + item.id + ', \'30\')"><i class="layui-icon">&#xe605;</i></button></td>')
                            html.append('<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.funcs.transferState(' + item.id + ', \'40\')"><i class="layui-icon">&#x1006;</i></button></td>')
                        }
    ...
    var funcs = {
        editData: function (id) {
            var saveBlogUrl = formatContextUrl('/static/admin/html/blog/addBlog.html?id=' + id + "&editType=admin")
...
function previewUrl(id, state) {
    if ("30" === state) {
        return formatContextUrl("/static/main/html/blog/blogDetail.html?id=" + id)
    } else {
        return formatContextUrl("/static/admin/html/blog/blogDetail.html?id=" + id)
    }
}
