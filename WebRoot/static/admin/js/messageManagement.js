...
layui.define(['element', 'laypage', 'layer', 'form', 'pagesize'], function (exports) {
            ...
            success: function (resp) {
                if (resp.success) {
                    ...
                    for (var i in resp.data.list) {
                        ...
                        html += '<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.funcs.editData(' + item.id + ', \'' + item.senderName + '\', \'' + item.createdAt + '\', \'' + item.type + '\', \'' + item.subject + '\', \'' + transferQuote(encodeURI(item.content)) + '\')"><i class="layui-icon">&#xe642;</i></button></td>';
    ...
    var funcs = {
        showData: function (id, senderName, createdAt, subject, content) {
            ...
            html += '<hr>'
            html += '<div style="width:87%;margin: auto;color: #000!important; height:200px" lay-verify="required" name="content" class="layui-area" readonly >' + decodeURI(detransferQuote(content)) + '</div>';
            html += '<hr>';
            html += '</div>';
            html += '</fieldset>';
            layer.open({
        ...
        addReply: function (id, senderId, senderName, createdAt, subject, content) {
            ...
            html += '<input type="hidden" name="type" value="siteInternal"/>';
        ...
        editData: function (id, senderName, createdAt, type, subject, content) {
            ...
            html += '<hr>'
            html += '<textarea style="width:87%;margin: auto;color: #000!important; height:200px" lay-verify="required" name="content" class="layui-area" >' + decodeURI(detransferQuote(content)) + '</textarea>';
            html += '<hr>';
            html += '</div>';
            html += '<div class="layui-form-item">';
            html += '<div class="layui-input-inline" style="margin:10px auto 0 auto;display: block;float: none;">';
